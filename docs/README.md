...
- [Kaggle Notebook](../raes-jax-celebahq-kaggle.ipynb): end-to-end CelebA-HQ notebook for the standard JAX branch flow, downloading `eurecom-ds/celeba-hq-256` from Hugging Face into a local cache, exporting it into `ImageFolder`, syncing repo dependencies into `/tmp/.venv` via `uv sync`, running package-backed steps through `uv run`, and enabling `training.random_flip=true`.
- [Kaggle TPU Notebook (SiTDH-S)](../raes-jax-celebahq-kaggle-tpuv5e8-sitdh-s.ipynb): CelebA-HQ notebook tuned for Kaggle `TPU v5e-8` for the `SiTDH-S` variant, also syncing repo dependencies into `/tmp/.venv` via `uv sync`, with an automatic `jaxlib` executable-stack fix, Hugging Face export into `ImageFolder`, host-side CPU FID/stat work, `training.random_flip=true`, and a default `210000`-step checkpoint cadence.
- [Kaggle TPU Notebook (SiTDH-B)](../raes-jax-celebahq-kaggle-tpuv5e8-sitdh-b.ipynb): sibling `TPU v5e-8` notebook that keeps the same `/tmp/.venv` + `uv sync` JAX/Kaggle flow and the same Hugging Face `celeba-hq-256` export step, but switches the CelebA-HQ Stage 2 recipe to the DH/two-tower `SiTDH-B` setup with `hidden_size=[768, 2048]`, `depth=[12, 2]`, `num_heads=[12, 16]`, `use_pos_embed=true`, `class_dropout_prob=0.0`, and `training.random_flip=true`, while keeping the `210000`-step checkpoint cadence.
- [Kaggle TPU Notebook (SiTDH-B Resume)](../raes-jax-celebahq-kaggle-tpuv5e8-sitdh-b-resume.ipynb): minimal `TPU v5e-8` resume-only notebook that starts by unzipping the previous notebook `_output_.zip` back into `/kaggle/working`, then rebuilds only the `uv` environment and auto-detects the latest `CelebAHQ256_SiTDH-B_DINOv2-B_jax_tpuv5e8-*` run plus its latest `checkpoint_<step>`, with strict W&B reuse plus automatic rewind to the latest checkpoint once the workdir has `wandb_run.json` or a one-time `--wandb-run-id` bind for legacy runs.
