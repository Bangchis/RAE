...
- [Kaggle Notebook](../raes-jax-celebahq-kaggle.ipynb): end-to-end CelebA-HQ notebook for the standard JAX branch flow, downloading `eurecom-ds/celeba-hq-256` from Hugging Face into a local cache, exporting it into `ImageFolder`, syncing repo dependencies into `/tmp/.venv` via `uv sync`, and running package-backed steps through `uv run`.
