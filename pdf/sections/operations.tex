...
\begin{itemize}[leftmargin=1.5em]
  ...
  \texttt{eurecom-ds/celeba-hq} từ Hugging Face và export sang
  \texttt{ImageFolder} bằng \texttt{src\_jax/export\_celebahq\_hf.py}, rồi
  chạy các bước phụ thuộc package qua \texttt{uv run}. Route TFDS cũ vẫn còn
  trong repo, nhưng chỉ đóng vai trò fallback khi cần source manual chính thức.
  ...
  tiến trình Python mới. Notebook này cũng tải \texttt{eurecom-ds/celeba-hq}
  từ Hugging Face rồi export sang \texttt{ImageFolder} trước khi build Stage 1 stat, và đã bỏ override chẩn đoán
