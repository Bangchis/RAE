...
Đường JAX vẫn tái sử dụng được file \texttt{reference\_stats.npz} này.
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item Online validation loss hiện có parity tốt ở đường XLA hơn đường JAX.
  \item FID trên đường JAX và XLA đều dùng reference stats của repo, nhưng
  backend JAX sẽ chuyển \texttt{.npz} sang pickle nội bộ khi cần.
  ...
  \texttt{raes-jax-celeba-kaggle-tpuv5e8.ipynb}; bản này chuyển FID và build
  reference stats sang host CPU thay vì \texttt{cuda}, đồng thời cũng giữ các
  bước JAX trong \texttt{.venv} riêng qua \texttt{uv run}, tự xóa cờ
  executable-stack mà Kaggle có thể chặn trên \texttt{jaxlib}, rồi kiểm tra
  \texttt{jax}/TPU bằng tiến trình Python mới.
