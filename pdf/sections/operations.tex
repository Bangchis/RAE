...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item Nếu chạy trên Kaggle với CelebA, bắt đầu từ notebook
  \texttt{raes-jax-celeba-kaggle.ipynb} để giữ đúng flow clone repo, đặt
  \texttt{UV\_PROJECT\_ENVIRONMENT=/tmp/.venv} và
  \texttt{UV\_CACHE\_DIR=/tmp/uv-cache}, chạy \texttt{uv sync -q}, rồi chạy các
  bước phụ thuộc package qua \texttt{uv run}.
  \item Nếu phần cứng là Kaggle \texttt{TPU v5e-8}, dùng notebook
  \texttt{raes-jax-celeba-kaggle-tpuv5e8-ditdh-s.ipynb}; bản này cố định biến
  thể Stage 2 là \texttt{DiTDH-S}, chuyển FID và build reference stats sang
  ...
  tiến trình Python mới. Notebook này cũng đã bỏ override chẩn đoán
  ...
  \texttt{DiTDH-B}, dùng notebook
  \texttt{raes-jax-celeba-kaggle-tpuv5e8-ditdh-b.ipynb}. Bản này giữ toàn bộ
  fix Kaggle/JAX hiện có, chỉ đổi phần Stage 2 sang config \texttt{DiTDH-B},
  và dùng cùng cadence checkpoint \texttt{210000} step.
  \item Nếu đã có sẵn một run Orbax của \texttt{DiTDH-B} và cần train tiếp từ
  checkpoint hiện có, dùng notebook
  \texttt{raes-jax-celeba-kaggle-tpuv5e8-ditdh-b-resume.ipynb}. Notebook này
