...
\begin{itemize}[leftmargin=1.5em]
  ...
  \texttt{raes-jax-celebahq-kaggle.ipynb} để giữ đúng flow clone repo, đặt
  \texttt{UV\_PROJECT\_ENVIRONMENT=/tmp/.venv} và
  \texttt{UV\_CACHE\_DIR=/tmp/uv-cache}, chạy \texttt{uv sync -q}, export TFDS
  \texttt{celeb\_a\_hq/256} sang \texttt{ImageFolder} bằng
  \texttt{src\_jax/export\_celebahq\_tfds.py}, rồi chạy các bước phụ thuộc
  package qua \texttt{uv run}. TFDS này vẫn cần các tar file manual chính thức.
  \item Nếu phần cứng là Kaggle \texttt{TPU v5e-8}, dùng notebook
  \texttt{raes-jax-celebahq-kaggle-tpuv5e8-sitdh-s.ipynb}; bản này cố định biến
  thể Stage 2 là \texttt{SiTDH-S}, chuyển FID và build reference stats sang
  ...
  tiến trình Python mới. Notebook này cũng export TFDS \texttt{celeb\_a\_hq/256}
  sang \texttt{ImageFolder} trước khi build Stage 1 stat, và đã bỏ override chẩn đoán
  \texttt{ema\_decay=0.99} và giữ mốc checkpoint mặc định ở \texttt{210000}
  step, nên mặc định quay lại EMA chuẩn của config.
  \item Nếu cần cùng flow Kaggle TPU đó nhưng muốn train biến thể
  \texttt{SiTDH-B}, dùng notebook
  \texttt{raes-jax-celebahq-kaggle-tpuv5e8-sitdh-b.ipynb}. Bản này giữ toàn bộ
  ...
  \texttt{use\_pos\_embed=true}, \texttt{class\_dropout\_prob=0.0}, và dùng
  cùng cadence checkpoint \texttt{210000} step.
  \item Nếu đã có sẵn một run Orbax của \texttt{SiTDH-B} và cần train tiếp từ
  checkpoint hiện có, dùng notebook
  \texttt{raes-jax-celebahq-kaggle-tpuv5e8-sitdh-b-resume.ipynb}. Notebook này
  ...
  lấy secret wandb, kiểm tra path, rồi tự tìm thư mục run
