...
Lớp \texttt{SiTDH} là wrapper repo-facing cho backbone single-tower
\texttt{LightningDiT} trong \texttt{src/stage2/models/lightningDiT.py}. File
\texttt{src/stage2/models/DDT.py} vẫn còn trong repo như một legacy path, nhưng
không còn là target Stage 2 mặc định của branch.
...
\begin{itemize}[leftmargin=1.5em]
  \item \texttt{SiTDH} $\rightarrow$ \texttt{lightning\_dit},
...
Với branch \texttt{jax-sit-dh}, checkpoint Stage 2 phải khớp shape của
