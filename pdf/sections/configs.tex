...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item \texttt{hidden\_size}, \texttt{depth}, \texttt{num\_heads};
  \item các toggle như \texttt{use\_rope}, \texttt{use\_rmsnorm},
  \texttt{use\_swiglu}, \texttt{wo\_shift}.
\end{itemize}

Với config mặc định của branch này, \texttt{target} sẽ là \texttt{SiTDH} và
adapter JAX sẽ ánh xạ nó sang backend \texttt{lightning\_dit}. Các target DDT
legacy vẫn còn được adapter hiểu, nhưng repo-owned config không còn dùng chúng.
...
\texttt{num\_workers}, \texttt{max\_batches}, và \texttt{eval\_model} sẽ điều
khiển held-out validation loop trong \texttt{src\_jax/train.py}. Với
