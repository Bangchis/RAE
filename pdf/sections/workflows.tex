...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item export TFDS \texttt{celeb\_a\_hq/256} thành \texttt{ImageFolder} \texttt{256x256};
...
Script đứng sau bước export là \texttt{src\_jax/export\_celebahq\_tfds.py}.
Đường train JAX hiện tại vẫn tiêu thụ \texttt{ImageFolder}, nên TFDS chỉ được
dùng để materialize dữ liệu đầu vào. Dataset TFDS này cũng cần các tar file
manual chính thức, nên notebook kỳ vọng chúng có sẵn trong input Kaggle riêng.
Helper này cũng ép \texttt{PROTOCOL\_BUFFERS\_PYTHON\_IMPLEMENTATION=python}
trước khi import TFDS để tránh lỗi Kaggle kiểu ``Descriptors cannot be created
directly'' khi protobuf trong môi trường không đồng nhất.
