...
\subsection{Notebook Kaggle cho CelebA-HQ}

Repo hiện có notebook \texttt{raes-jax-celebahq-kaggle.ipynb} để bám đúng flow
Kaggle:

\begin{itemize}[leftmargin=1.5em]
  \item clone repo và checkout branch \texttt{jax-sit-dh-celebahq256};
  ...
  \item tải dataset Hugging Face \texttt{eurecom-ds/celeba-hq-256} rồi export thành \texttt{ImageFolder} \texttt{256x256};
  ...
  \item ghi sẵn config CelebA-HQ cho \texttt{src\_jax/train.py}.
\end{itemize}

Script mặc định đứng sau bước export là \texttt{src\_jax/export\_celebahq\_hf.py}.
Đường train JAX hiện tại vẫn tiêu thụ \texttt{ImageFolder}, nên notebook chỉ
dùng Hugging Face để tải \texttt{eurecom-ds/celeba-hq-256} vào cache cục bộ rồi
materialize dữ liệu đầu vào tại chỗ. Nếu vẫn muốn đi theo route TFDS cũ, repo
giữ lại \texttt{src\_jax/export\_celebahq\_tfds.py}; script đó vẫn cần các tar
file manual chính thức và vẫn ép
\texttt{PROTOCOL\_BUFFERS\_PYTHON\_IMPLEMENTATION=python} trước khi import TFDS
để tránh lỗi Kaggle kiểu ``Descriptors cannot be created directly'' khi
protobuf trong môi trường không đồng nhất.

Cell train Stage 2 cuối của các notebook CelebA-HQ truyền
\texttt{--data-path /kaggle/working/celebahq256\_imgfolder}, còn
\texttt{eval.data\_path} trong config sinh ra vẫn trỏ tới split
\texttt{val}. Các notebook này mặc định bật
\texttt{training.random\_flip=true}, giữ \texttt{eval.random\_flip=false}, và
các bản TPU còn tăng \texttt{training.prefetch\_factor=8} cùng
\texttt{eval.prefetch\_factor=4}.

Nếu chạy trên Kaggle \texttt{TPU v5e-8}, dùng notebook
\texttt{raes-jax-celebahq-kaggle-tpuv5e8-sitdh-s.ipynb}. Bản này cố định biến thể
CelebA-HQ Stage 2 là \texttt{SiTDH-S}, sync dependency của repo vào
...
\texttt{210000} step. Config train CelebA-HQ của notebook này cũng bật
\texttt{training.random\_flip=true}.

Nếu muốn giữ nguyên flow đó nhưng đổi Stage 2 sang biến thể DH
\texttt{SiTDH-B},
dùng notebook \texttt{raes-jax-celebahq-kaggle-tpuv5e8-sitdh-b.ipynb}. Notebook
này ghi config CelebA-HQ thành
\texttt{CelebAHQ256\_SiTDH-B\_DINOv2-B\_jax\_tpuv5e8.yaml}, đặt backbone DH thành
\texttt{hidden\_size=[768, 2048]}, \texttt{depth=[12, 2]},
\texttt{num\_heads=[12, 16]}, bật \texttt{use\_pos\_embed}, tắt label dropout
bằng \texttt{class\_dropout\_prob=0.0}, bật
\texttt{training.random\_flip=true}, đặt tên run/project mặc định theo biến thể
\texttt{SiTDH-B}, và
giữ cùng cadence checkpoint \texttt{210000} step.

Nếu đã có thư mục run Orbax của \texttt{SiTDH-B} và cần train tiếp, dùng
notebook \texttt{raes-jax-celebahq-kaggle-tpuv5e8-sitdh-b-resume.ipynb}. Notebook
...
\texttt{CelebAHQ256\_SiTDH-B\_DINOv2-B\_jax\_tpuv5e8-*} mới nhất bên dưới
...
\texttt{--workdir} vào \texttt{src\_jax/train.py}. Nếu workdir đích là legacy
run được tạo trước khi cơ chế \texttt{wandb\_run.json} tồn tại, cần thêm
\texttt{--wandb-run-id <existing\_run\_id>} một lần để notebook resume đúng
chính run W\&B cũ đó. Runtime tiếp tục restore \texttt{latest\_step()} từ
checkpoint mới nhất, nên notebook này không còn hardcode cả thư mục run có
timestamp lẫn \texttt{checkpoint\_100000}.
